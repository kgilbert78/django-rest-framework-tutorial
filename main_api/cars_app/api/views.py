...
def drive(request):
    ...
    if request.query_params:
        ...
        )  # create list of the keys in param QueryDict
        if "id" in params:
...
class CarViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if "-" in params["pk"]:
            ...
            selected_cars = Car.objects.filter(
                car_brand=param_list[0], car_model=param_list[1]
            )
        else:
            selected_cars = Car.objects.filter(car_brand=params["pk"])
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if str(logged_in_user) == "admin":
            ...
            response_message = {
                "message": f"{color} {year} {brand} {model} has been deleted"
            }
        else:
            response_message = {
                "message": "You do not have authorization to delete this."
            }
